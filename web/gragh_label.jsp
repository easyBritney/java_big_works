...
<%@ page import="hibernate_test.json.VerticalBPJsonGenerate" %>
...
<script>
    <%
        file = new File("C:/source/javabigwork/out/artifacts/javabigwork_war_exploded/data/verticalBP.json");
        if(!file.exists())
            VerticalBPJsonGenerate.createJson();
    %>
    ...
    d3.json("data/verticalBP.json", function (error, tagCloud) {
        if (error) throw error;
        ...
        for (let item in tmp) {
            ...
            for (let city in tmp[item]) {
                data_tmp = [];
                // console.log(item);
                // console.log(city);
                // console.log(tmp[item][city])
                data_tmp.push(item);
                data_tmp.push(city);
                data_tmp.push(tmp[item][city]);
                data.push(data_tmp)
            }
        }
